...
以下名称和默认逻辑出口由两个客户端共同维护，不得在适配器中使用别名：

| 策略组 | 默认逻辑出口 |
|---|---|
| `🚀 节点选择` | 当前订阅或 Clash 传输策略 |
| `🔗 全局直连` | `DIRECT` |
| `❌ 全局拦截` | `REJECT` |
| `🇭🇰 香港节点`、`🇹🇼 台湾节点`、`🇯🇵 日本节点`、`🇺🇸 美国节点`、`🌐 其他节点` | 对应地区自动测速 |
| `🛑 广告拦截` | `❌ 全局拦截` |
| `💸 OpenAI`、`💵 Claude`、`🧠 XAI`、`🔎 Perplexity` | `🇺🇸 美国节点` |
| `🔍 谷歌服务` | `🇯🇵 日本节点`，香港节点作为第二选择 |
| `📈 券商服务` | `🇭🇰 香港节点` |
| `🍏 苹果服务`、`🏠 私有网络`、`🔒 国内服务` | `🔗 全局直连` |
| `📹 油管视频`、`Ⓜ️ 微软服务`、`🍎 苹果推送`、`📲 电报消息`、`🐱 代码托管` | `🚀 节点选择` |
| `🌍 非中国`、`🐟 漏网之鱼` | 代理出口；Clash 使用 `🚀 节点选择`，Shadowrocket 使用内置 `PROXY` |

Clash 的落地节点、延迟选优、故障转移和负载均衡属于客户端专属传输策略，可以作为 `🚀 节点选择` 的候选项，但不得替代上述公共名称。DNS 是基础设施配置；HTTPDNS 和 WebRTC 防泄漏规则直接使用 `REJECT`，不得创建可切换的“DNS 防泄漏”策略组。
